...
library(leaflet)
library(geosphere)
#install.packages("Hmisc") #teksti muutmiseks
library(Hmisc)

#Statistilise andmeteaduse projekt
#Autoorid: Diana Sokurova ja Linnet Puskar

#Loen andmestikud sisse, otse leheküljest. Võtan umbes 50 sec projekti avamiseks
...
avalik_1<-mutate(avalik_1, 
                 Informatsioon=paste(avalik_1$SyndmusLiik_ilus, format(avalik_1$ToimKpv,"%d.%m.%Y"), sep=", "))

unique(avalik_1$SyndmusLiik_ilus)

#liiklusjarelevalve_1
...
liiklusjarelevalve_1<-mutate(liiklusjarelevalve_1, 
                             Informatsioon=paste(liiklusjarelevalve_1$SoidukLiik_ilus, format(liiklusjarelevalve_1$ToimKpv,"%d.%m.%Y"), sep=", ")
)
...
unique(vara_1$SyndmusLiik_ilus)
vara_1<-mutate(vara_1, 
               Informatsioon=paste(vara_1$SyndmusLiik_ilus, format(vara_1$ToimKpv,"%d.%m.%Y"), sep=", "))
    ...
    fluidRow(
      tabBox(
        ...
        dateRangeInput('dateRange',
                         ...
                         start = Sys.Date() - 14, end = Sys.Date(), weekstart = 1, language = "et", separator = "kuni"
                      ),
...
server <- function(input, output) {
  ...
  output$Kaart<-renderLeaflet({
    ...
    #Kui on tehtud linnuke avalike süütegude kasti juurde, siis kantakse graafikule avalikes kohtades toimepandud sÃÂ¼ÃÂ¼teod.
    if("avalik" %in% input$checkGroup){
      ...
      map<-map %>%
        addAwesomeMarkers(lng = alamandmestik$lon1, lat = alamandmestik$lat1, icon = awesomeIcons(icon = 'fa-users', library = 'fa', markerColor = 'green'),  popup=alamandmestik$Informatsioon)
